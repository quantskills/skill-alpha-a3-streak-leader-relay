...
- ✅ **OOS 样本外有效**：近 6 月 IC=0.118 / ICIR=0.150，达学界可投资门槛 (ICIR > 0.10)。
- ✅ **OOS 纯 A3 策略正收益**：ARR +40.9% / 累计 +10.3% / 夏普 0.96 / 胜率 48.4%（标准 0.30% 双成本）。
- ✅ **当前环境对 A3 友好**：2026 年 4-6 月 OOS 月度 -20.7% / +15.2% / +84.3%。
- ⚠️ **环境敏感**：IS IC=0.008 vs OOS IC=0.118，因子高度依赖游资活跃环境。
- ⚠️ **成本敏感**：0.30% → 0.50% 双成本直接翻负（+40.9% → -13.7%），实盘费率必须 < 0.30%。
- 💡 **IC gate 当前冗余**：子因子已内化游资活跃度信息，纯 A3 + 人工选时即可。

**定位**：候选发现器（不是黑盒策略），建议 paper trading 验证 1-2 个月后再实盘。完整可视化报告见 `alpha-a3-streak-leader-relay-production/backtest_report.html`。
