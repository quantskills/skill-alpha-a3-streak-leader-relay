...
- ✅ **OOS 含 IC gate + 大盘情绪 + score 加权 = 当前最优**：ARR +220% / 累计 +22.8% / 夏普 1.52 / 胜率 50.0% / MDD -34%（标准 0.30% 双成本）。
- ✅ **mkt_state 与 IC gate 强协同**：单独使用任一都拖累纯 A3，组合后 ARR 反弹 +281%。
- ⚠️ **裸用纯 A3 不推荐**：OOS 纯 A3 ARR -47.6%，必须配合 IC gate + 大盘情绪标量过滤。
- ⚠️ **成本敏感**：0.30% → 0.50% 仍敏感，实盘费率必须 < 0.30%。
